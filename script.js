...
function answer(btn,label,isCorrect,q){
 ...
 if(isCorrect){score++;btn.classList.add("correct");$("#feedback").classList.add("feedback-correct");$("#feedback").classList.remove("feedback-wrong");$("#feedback").innerHTML='<div class="fb-title">Rätt</div>';}
   ...
   if(currentQuiz==="reading"){
     ...
     $("#feedback").innerHTML=`<div class="concept-fb-heading">FEL</div><div class="concept-fb-correctline">Rätt svar är <strong>${q.answer}</strong>.</div><div class="concept-fb-explanation">${correctFeedback(q.answer,q.kind)}</div>`;
   }
 }
 $("#feedback").classList.remove("hidden");$("#nextBtn").classList.remove("hidden");
}

function correctFeedback(correct,kind){
 if(kind==="level"){
   return correct==="Mikronivå"
     ? "<strong>Mikronivå</strong> betyder sociologi på individnivå och fokuserar på individer och direkt socialt samspel mellan människor."
     : correct==="Mesonivå"
     ? "<strong>Mesonivå</strong> betyder sociologi på gruppnivå och fokuserar på grupper, organisationer och andra sociala gemenskaper."
     : "<strong>Makronivå</strong> betyder sociologi på strukturnivå (samhällsnivå) och fokuserar på samhällets övergripande strukturer, institutioner och system.";
 }
 const feedback={
  "Socialisation":"<strong>Socialisation</strong> är processen där individen lär sig samhällets normer, värderingar och beteenden. Det handlar om vad vi lär oss genom exempelvis familj, skola, vänner och medier.",
  "Social interaktion":"<strong>Social interaktion</strong> innebär kommunikation, samspel och ömsesidig påverkan mellan människor. Det avgörande är att människor påverkar varandra i ett socialt samspel.",
  "Internalisering":"<strong>Internalisering</strong> innebär att andras tankar, värderingar och reaktionssätt införlivas i den egna personen. Det som kommer från omgivningen blir alltså en del av individens eget sätt att tänka, känna eller se på sig själv.",
  "Konformitet":"<strong>Konformitet</strong> innebär att individen anpassar sina beteenden eller åsikter till en grupp. Till skillnad från internalisering behöver gruppens uppfattning inte bli en del av individens egen övertygelse – individen kan anpassa sig för att passa in eller undvika att avvika.",
  "Social konstruktion":"<strong>Social konstruktion</strong> är de föreställningar och betydelser som skapas gemensamt av människor i ett samhälle. De uppfattas ofta som självklara, men är socialt skapade och kan därför förändras mellan samhällen och över tid.",
  "Social kontroll":"<strong>Social kontroll</strong> är samhällets och gruppers sätt att få människor att följa normer och regler. Begreppet omfattar både formell kontroll genom exempelvis lagar och informell kontroll genom sociala reaktioner.",
  "Formell social kontroll":"<strong>Formell social kontroll</strong> innebär social kontroll genom lagar, myndigheter och formella regler. Den skiljer sig från informell social kontroll genom att reglerna och sanktionerna är formellt fastställda.",
  "Informell social kontroll":"<strong>Informell social kontroll</strong> innebär social kontroll genom oskrivna normer och andra människors reaktioner. Den sker exempelvis genom blickar, kritik, beröm eller risken att hamna utanför gruppen.",
  "Mikronivå":"<strong>Mikronivå</strong> betyder sociologi på individnivå och fokuserar på individer och direkt socialt samspel mellan människor.",
  "Mesonivå":"<strong>Mesonivå</strong> betyder sociologi på gruppnivå och fokuserar på grupper, organisationer och andra sociala gemenskaper.",
  "Makronivå":"<strong>Makronivå</strong> betyder sociologi på strukturnivå (samhällsnivå) och fokuserar på samhällets övergripande strukturer, institutioner och system."
 };
...
let conceptCardOrder = Object.entries(CONCEPT_DEFS);

function createConceptShuffleControl(){
 const g=$("#cardsGrid");
 if(!g || document.getElementById("concept-shuffle-button")) return;
 ...
 button.innerHTML=`
  <span class="shuffle-icon" aria-hidden="true">
   <svg viewBox="0 0 24 24" focusable="false">
    <path d="M7.1 7.1A7 7 0 0 1 18 9"></path>
    <path d="M18 5v4h-4"></path>
    <path d="M16.9 16.9A7 7 0 0 1 6 15"></path>
    <path d="M6 19v-4h4"></path>
   </svg>
  </span>
  <span>BLANDA ORDNINGEN</span>`;
 button.onclick=()=>{
   conceptCardOrder=shuffle(conceptCardOrder);
   renderCards(false);
 };
 controls.appendChild(button);
 g.parentNode.insertBefore(controls,g);
}

function renderCards(openScreen=true){
 const g=$("#cardsGrid");
 createConceptShuffleControl();
 g.innerHTML="";
 conceptCardOrder.forEach(([term,def])=>{
